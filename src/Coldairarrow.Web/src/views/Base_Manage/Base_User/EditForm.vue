  ...
  <a-modal title="编辑表单" width="40%" :visible="visible" :confirmLoading="confirmLoading" @ok="handleSubmit" @cancel="()=>{this.visible=false}">
    <a-spin :spinning="confirmLoading">
      <a-form-model ref="form" :model="entity" :rules="rules" v-bind="layout">
        ...
        <a-form-model-item label="项目地" prop="ProjectIdList">
          <a-select v-model="entity.ProjectIdList" allowClear mode="multiple">
            <a-select-option v-for="item in ProjectOptionList" :key="item.Id">{{ item.Project_Name }}</a-select-option>
          </a-select>
        </a-form-model-item>
      </a-form-model>
    </a-spin>
  ...
  export default {
    ...
    methods: {
      init() {
        ...
        this.$http.post('/MiniPrograms/mini_project/GetDataList', {}).then(resJson => {
          if (resJson.Success) {
            this.ProjectOptionList = resJson.Data
          }
        })
      },
      openForm(id) {
